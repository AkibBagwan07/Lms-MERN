...
const AddCourse = () => {
  ...
  console.log(chapters.chapter?.chapterTitle)
  useEffect(() => {
  ...
  return (
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0'>
      <form onSubmit={()=>handleSubmit()} className='flex flex-col gap-4 max-w-md w-full file-gray-500'>
        <div className='flex flex-col gap-1'>
          <p>Course Title</p>
          <input onChange={(e) => setCourseTitle(e.target.value)} value={courseTitle} placeholder='Type here'
            className='outline-none md:py-2.5 py-2 px-3 rounded border border-gray-500' type="text" required/>
        </div>
        <div className='flex flex-col gap-1'>
        ...
        <div className='flex items-center justify-between flex-wrap'> 
          <div className='flex flex-col gap-1'>
            <p>Course Price</p>
            <input type="number" onChange={(e) => setCoursePrice(e.target.value)} placeholder='0'
          ...
          <div className='flex md:flex-row flex-col items-center gap-3'>
            ...
            <label htmlFor="thumbnailImage" className='flex items-center gap-3'>
              ...
              {image && <img src={image ? URL.createObjectURL(image) : ""} alt="uploaded_image" className='max-h-10' />}
            </label>
          </div>
        </div>
        <div className='flex flex-col gap-1'>
          <p>Discount%</p>
          <input type="number" value={discount} placeholder="0" min={0} max={100}
          className='outline-none md:py-2.5 w-28 px-3 rounded border border-gray-500' required onChange={(e)=>setDiscount(e.target.value)}/>
        </div>
        {/* adding chapters and lecs */}
        <div>
          {chapters.map((chapter, idx) => (
            <div key={idx} className='bg-white border rounded-lg mb-4'>
              <div className="flex justify-between items-center p-4 border-b">
                <div className='flex items-center'>
                  <img onClick={()=>handleChapter("toggle" , chapter.chapterId)} src={assets.dropdown_icon} alt="dropdown_icon" width={14}
                    className={`mr-2 cursor-pointer transition-all ${chapter.collapsed && "-rotate-90"}`} />
                  <span className='font-semibold'>{idx + 1} { chapter.chapterTitle}</span>
                </div>
                ...
                <div className='p-4'>
                  {chapter.chapterContent.map((lecture, idx) => (
                    <div className='flex justify-between items-center mb-2' key={idx}>
                      <span>{idx + 1} {lecture.lectureTitle} - {lecture.lectureDuration} mins -
                        <a href={lecture.lectureUrl} target='_blank' className='text-blue-500'>Link</a>
                        -{lecture.isPreviewFree ? "Free Preview" : "Paid"}</span>
                      <img onClick={()=>handleLecture("remove",chapter.chapterId,idx)} src={ assets.cross_icon} className='cursor-pointer' alt="" />
                    </div>
                  ))}
