           ...
           overflow-hidden rounded-md bg-white border border-gray-500/20'>
            <table className='table-fixed md:table-auto w-full overflow-hidden '>
              ...
              <tbody className='text-sm text-gray-500'>
                {dashboardData.enrolledStudentsData.map((data, idx) => (
                  <tr className='border-b border-gray-500/20' key={idx}>
                    <td className='px-4 py-3 text-center hidden sm:table-cell '>
                      { idx + 1}
                    </td>
                    <td className='md:px-4 px-2 py-3 flex items-center space-x-3'>
                      <img src={ data.student.imageUrl} alt="image" className='w-9 h-9 rounded-full'/>
