...
const CourseDetails = () => {
  ...
  const { allCourses ,calculateRating ,
    ...
    calculateNoOfLectures, calculateChapterTime ,currency} = useContext(AppContext)
      ...
      <div className='max-w-xl z-10 text-gray-500'>
         ...
         font-semibold text-gray-800'>{courseData.courseTitle}</h1>
          ...
          <div className='flex items-center space-x-2 pt-3 pb-1 text-sm'>
            ...
            <p className='text-blue-600'>({courseData.courseRatings.length} { courseData > 1 ? "Ratings" : "Rating"})</p>
            <p>{ courseData.enrolledStudents.length} { courseData.enrolledStudents.length > 1 ? "Students" : "Student"}</p> 
          </div>
          <p className='text-sm'>Course by <span className='text-blue-600 underline'>Learnary</span></p>
            ...
            <div className='pt-5 '>
              {courseData.courseContent.map((chapter, idx) => (
                <div className=' border border-gray-300 bg-white mb-2 rounded ' key={idx}>
                  <div onClick={()=>toggleSection(idx)} className='cursor-pointer bg-gray-500/10 flex items-center justify-between px-4 py-3 cursoe-pointer select-none'>
                    ...
                    <ul className='list-disc md:pl-10 pl-4 pr-4 py-2 text-gray-600 border-t border-gray-300'>
                      {chapter.chapterContent.map((lecture, idx) => (
                        <li key={idx} className='flex items-start gap-4 py-1'>
                          <img className='w-4 h-4 mt-1' src={assets.play_icon} alt="play-button" />
        ...
        <div className='max-w-course-card z-10 shadow-custom-card rounded-t md:rounded-none overflow-hidden 
          ...
          <div className='p-5'>
            ...
            <div className='flex gap-3 items-center pt-2'>
              ...
              <p className='md:text-l text-gray-500 line-through'>{currency} {courseData.coursePrice}</p>
              <p className='md:text-l text-gray-500'>{ courseData.discount} % off</p>
            </div>
